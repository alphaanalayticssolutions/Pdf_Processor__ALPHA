...
// ─────────────────────────────────────────────────────────────
// UPLOAD DROP ZONE (shared component)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// FILE LIST (shared component) — with overflow fix
// ─────────────────────────────────────────────────────────────
        ...
        return (
          <div
            ...
            style={{
              ...
              cursor: 'pointer',
              minWidth: 0, // critical for flex overflow fix
            }}
          >
            {/* Checkbox */}
            <div style={{
              width: '17px', height: '17px', borderRadius: '4px', flexShrink: 0,
              border: `2px solid ${isSelected ? '#1a3c6e' : '#d1d5db'}`,
...
// ─────────────────────────────────────────────────────────────
// FILE SELECTION PANEL (header + FileList combined)
// ─────────────────────────────────────────────────────────────
    ...
    <div style={{ marginBottom: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button onClick={onClear} style={{ background: 'none', border: '1px solid #d1d5db', color: '#6b7280', borderRadius: '20px', padding: '3px 12px', fontSize: '11px', fontWeight: '700', cursor: 'pointer' }}>
            Clear
          </button>
        </div>
      </div>
      ...
      {!someChecked && (
        <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '6px' }}>⚠️ Please select at least one file.</p>
      )}
    </div>
  );
}
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
        <span style={{ background: '#f0f4ff', color: '#1e40af', borderRadius: '8px', padding: '3px 10px', fontSize: '11px', fontWeight: '800', letterSpacing: '0.5px' }}>
          STEP {step}
        </span>
        <h2 style={S.sectionTitle}>{icon} {title}</h2>
        {badge && (
          <span style={{ background: '#fef3c7', border: '1px solid #fcd34d', color: '#92400e', borderRadius: '20px', padding: '3px 10px', fontSize: '11px', fontWeight: '700' }}>
            🤖 AI-Powered
          </span>
        )}
      </div>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '36px', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} onKeyDown={handleKeyDown}
                ...
                style={{ ...S.inputBase, paddingRight: '46px' }}
                onFocus={e => e.target.style.borderColor = '#1a3c6e'}
                onBlur={e => e.target.style.borderColor = '#e5e7eb'} />
          ...
          <div style={{ marginTop: '20px', padding: '12px', background: '#f8fafc', borderRadius: '10px', textAlign: 'center' }}>
            <p style={{ color: '#94a3b8', fontSize: '11px', margin: '0', lineHeight: '1.6' }}>
              🔒 Access restricted to <strong style={{ color: '#475569' }}>@alphaanalyticssol.com</strong> accounts only.
            </p>
          </div>
        </div>

        <p style={{ textAlign: 'center', color: '#334155', fontSize: '11px', marginTop: '20px', letterSpacing: '1px' }}>
          © 2025 ALPHA ANALYTICS SOLUTIONS • CONFIDENTIAL
        </p>
      </div>
    </main>
  );
...
// ─────────────────────────────────────────────────────────────
// STEP NAVIGATION BAR — 2-row, no scrolling, all visible
// ─────────────────────────────────────────────────────────────
      ...
      <div style={{ display: 'flex', alignItems: 'center', flex: 1, minWidth: 0 }}>
        <button
          ...
          style={{
            ...
            padding: '7px 10px',
            borderRadius: '8px',
            background: hover ? '#1a3c6e' : '#f0f4ff',
            border: `1px solid ${hover ? '#1a3c6e' : '#dce6ff'}`,
            cursor: 'pointer',
        ...
        >
          <span style={{
            background: hover ? 'rgba(255,255,255,0.2)' : '#1a3c6e',
            color: 'white',
            borderRadius: '6px',
            width: '20px',
            height: '20px',
        ...
        {!isLast && (
          <span style={{ color: '#c8d0dc', fontSize: '14px', flexShrink: 0, margin: '0 4px' }}>→</span>
        )}
      </div>
    );
  };
    ...
    <div style={{ background: 'white', borderBottom: '1px solid #e8ecf0', padding: '10px 20px' }}>
      <div style={{ maxWidth: '960px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '6px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          {row1.map((t, i) => (
            <StepChip key={t.id} t={t} isLast={i === row1.length - 1} />
          ))}
        </div>
        {/* Row 2 */}
        {row2.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            {row2.map((t, i) => (
              <StepChip key={t.id} t={t} isLast={i === row2.length - 1} />
            ))}
          </div>
        )}
      </div>
...
export default function Home() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f1f5f9', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0c1a30 0%, #0f2444 60%, #162d56 100%)', padding: '44px 20px 36px', textAlign: 'center', position: 'relative' }}>
        <div style={{ position: 'absolute', top: '16px', right: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button onClick={handleLogout}
            style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', color: 'white', padding: '6px 16px', borderRadius: '20px', fontSize: '12px', cursor: 'pointer', fontWeight: '600' }}>
            Sign Out
          </button>
        </div>
      ...
      {/* ── STEP FLOW BAR — 2 rows, no scroll ── */}
      {!activeTool && <StepNavBar tools={tools} onSelect={setActiveTool} />}

      {/* ── TOOL CARDS GRID — no step badges ── */}
      {!activeTool && (
...
// ─────────────────────────────────────────────────────────────
// TOOL CARD (no step badge — clean look)
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <div
      ...
      style={{
        ...
        border: hover && tool.active ? `2px solid ${tool.color}` : '2px solid #e5e7eb',
        boxShadow: hover && tool.active ? `0 8px 24px ${tool.color}18` : '0 1px 4px rgba(0,0,0,0.06)',
    ...
    >
      {/* Top accent line */}
      {tool.active && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: tool.color, borderRadius: '14px 14px 0 0', opacity: hover ? 1 : 0.6, transition: 'opacity 0.18s' }} />
      )}

      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
        <span style={{ fontSize: '30px', lineHeight: 1 }}>{tool.icon}</span>
        {!tool.active && (
          <span style={{ background: '#f3f4f6', color: '#9ca3af', borderRadius: '20px', padding: '2px 10px', fontSize: '10px', fontWeight: '700' }}>SOON</span>
        )}
      </div>
...
function TransactionAnalysisTool({ onBack }) {
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      const blob = new Blob([Uint8Array.from(atob(data.excelFile), c => c.charCodeAt(0))], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  ...
  return (
    <div style={S.card}>
      ...
      <FileSelectionPanel
        files={allFiles} selected={selected}
        onToggle={n => setSelected(p => ({ ...p, [n]: !p[n] }))}
        onToggleAll={toggleAll} onClear={clearAll}
        label="Files" iconFn={iconFn}
      />

      {error && <div style={S.errorBox}>❌ {error}</div>}

      <button onClick={handleAnalyse} disabled={loading || selectedFiles.length === 0} style={{ ...S.primaryBtn(loading || selectedFiles.length === 0), marginBottom: '16px' }}>
  ...
  return (
    <div style={S.card}>
      ...
      <FileSelectionPanel
        ...
        onToggle={n => setSelected(p => ({ ...p, [n]: !p[n] }))}
        onToggleAll={toggleAll} onClear={clearAll}
        label="Excel Files" iconFn={() => '📊'}
      />

      {error && <div style={S.errorBox}>❌ {error}</div>}

      <button onClick={handleGenerate} disabled={loading || selectedFiles.length === 0} style={{ ...S.primaryBtn(loading || selectedFiles.length === 0), marginBottom: '20px' }}>
...
function DescriptionCategoriserTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      <button onClick={handleCategorise} disabled={loading || selectedFiles.length === 0} style={{ ...S.primaryBtn(loading || selectedFiles.length === 0), marginBottom: '20px' }}>
...
function DuplicateTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      <UploadZone
        icon={files.length > 0 ? '✅' : '📂'}
        title={files.length > 0 ? `${files.length} files ready` : 'Click to select folder'}
        subtitle={files.length > 0 ? 'Click to change selection' : 'All file types supported'}
        color={files.length > 0 ? '#15803d' : '#1a3c6e'}
        folder multiple
      ...
      <button onClick={handleSubmit} disabled={processing || files.length === 0} style={{ ...S.primaryBtn(processing || files.length === 0), marginTop: '20px' }}>
      ...
      {result?.success && (
        <div style={{ marginTop: '24px' }}>
          ...
          <button onClick={() => downloadExcel(result.excelFile)} style={S.successBtn}>⬇ Download Excel Report</button>
        </div>
      )}
    </div>
  );
...
function SplitterTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      <UploadZone
        icon={file ? '✅' : '📂'}
        title={file ? file.name : 'Click to select PDF'}
        subtitle={file ? 'Click to change' : 'Single .pdf file'}
        color={file ? '#15803d' : '#1a3c6e'}
        accept=".pdf"
      ...
      <div style={{ marginTop: '20px', marginBottom: '16px' }}>
        ...
        <ToggleGroup cols={4}
          ...
          options={[{ value: 'auto', label: '🤖 Auto' }, { value: 'bank', label: '🏦 Bank' }, { value: 'invoice', label: '🧾 Invoice' }, { value: 'tax', label: '📑 Tax' }]}
        />
      </div>

      <div style={{ marginBottom: '20px' }}>
        <label style={S.label}>How to Split?</label>
        <ToggleGroup value={splitMode} onChange={setSplitMode}
          options={[{ value: 'ai', label: '🤖 AI decides' }, { value: 'manual', label: '✏️ Manual pages' }]}
        />
      </div>
      ...
      <button onClick={handleSubmit} disabled={processing} style={S.primaryBtn(processing)}>
        ...
        <div style={{ marginTop: '24px' }}>
          <div style={{ background: '#f0fff4', border: '1px solid #86efac', borderRadius: '10px', padding: '14px', marginBottom: '14px' }}>
            ...
            {result.documents.map((doc, i) => (
              <p key={i} style={{ color: '#4b5563', fontSize: '13px', margin: '3px 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>• {doc.name} ({doc.pages} pages)</p>
            ))}
          </div>
          <button onClick={() => downloadZip(result.zipFile)} style={S.successBtn}>⬇ Download Split Documents (ZIP)</button>
...
function CategoriseTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      <UploadZone
        icon={files.length > 0 ? '✅' : '📂'}
        title={files.length > 0 ? `${files.length} files selected` : 'Click to select folder'}
        subtitle={files.length > 0 ? 'Click to change' : 'All file types supported'}
        color={files.length > 0 ? '#15803d' : '#1a3c6e'}
        folder multiple onChange={handleFolderSelect}
      />
              ...
              {result.categorizationResults.map((r, i) => (
                <div key={i}>
                  ...
                  {expandedRow === i && r.notes && (
                    <div style={{ background: '#fffbeb', padding: '10px 14px', borderBottom: '1px solid #fde68a', fontSize: '12px', color: '#92400e' }}>⚠️ {r.notes}</div>
                  )}
                </div>
              ))}
            </div>
...
function StampingTool({ onBack }) {
  ...
  const ToggleBtn = ({ val, cur, set, label }) => (
    <button onClick={() => set(val)}
      style={{ padding: '9px 8px', borderRadius: '8px', border: `2px solid ${cur === val ? '#1a3c6e' : '#e5e7eb'}`, background: cur === val ? '#eff6ff' : 'white', color: cur === val ? '#1a3c6e' : '#6b7280', fontSize: '12px', fontWeight: cur === val ? '700' : '400', cursor: 'pointer', transition: 'all 0.15s' }}>
      {label}
    </button>
  );
    ...
    <div style={S.card}>
      ...
      <div style={{ marginBottom: '16px', position: 'relative' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <button onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: '16px', padding: 0 }}>
            {showPassword ? '🙈' : '👁️'}
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '14px' }}>
        <div>
          ...
          <input type="text" value={prefix} onChange={e => setPrefix(e.target.value)} placeholder="DOC-" style={S.inputBase} />
        </div>
        <div>
          <label style={S.label}>Start Number</label>
          <input type="number" value={startNumber} onChange={e => setStartNumber(e.target.value)} style={S.inputBase} />
        </div>
        <div>
          <label style={S.label}>Pad Length</label>
          <input type="number" value={padLength} onChange={e => setPadLength(e.target.value)} style={S.inputBase} />
        </div>
      </div>
      ...
      {showAdvanced && (
        <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '20px', marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            <label style={S.label}>Stamp Color</label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              <ToggleBtn val="black" cur={stampColor} set={setStampColor} label="⚫ Black" />
              <ToggleBtn val="red" cur={stampColor} set={setStampColor} label="🔴 Red" />
            </div>
          </div>
          <div>
            <label style={S.label}>Font</label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
              <ToggleBtn val="Helvetica" cur={stampFont} set={setStampFont} label="Helvetica" />
              <ToggleBtn val="Courier"   cur={stampFont} set={setStampFont} label="Courier" />
              <ToggleBtn val="Times"     cur={stampFont} set={setStampFont} label="Times" />
            </div>
          </div>
          <div>
            <label style={S.label}>Corner Zone: {cornerPct}%</label>
            ...
            <p style={{ color: '#9ca3af', fontSize: '11px', margin: '4px 0 0' }}>AI checks the outer {cornerPct}% of each corner for existing content.</p>
          </div>
        </div>
      )}
      ...
      <button onClick={handleSubmit} disabled={processing || selectedFiles.length === 0} style={S.primaryBtn(processing || selectedFiles.length === 0)}>
      ...
      {result?.success && (
        <div style={{ marginTop: '28px' }}>
          ...
          <button onClick={() => downloadZip(result.zipFile)} style={S.successBtn}>⬇ Download Stamped PDFs (ZIP)</button>
...
function InvoiceExtractTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      <UploadZone
        icon={files.length > 0 ? '✅' : '📂'}
        title={files.length > 0 ? `${files.length} invoice files ready` : 'Click to select folder'}
        subtitle={files.length > 0 ? 'Click to change' : 'PDF files only'}
        color={files.length > 0 ? '#15803d' : '#1a3c6e'}
        folder multiple onChange={handleFolderSelect}
      />
      ...
      <div style={{ marginTop: '20px', marginBottom: '20px' }}>
        ...
        <textarea value={fields} onChange={e => setFields(e.target.value)} rows={3} placeholder="Invoice Date, Invoice Number, Customer Name, Amount…"
          style={{ ...S.inputBase, resize: 'vertical', lineHeight: '1.5' }} />
      </div>
      ...
      {result && !result.success && (
        <div style={{ marginTop: '16px', ...S.errorBox }}>❌ Error: {result.error}</div>
      )}
    </div>
  );
}
...
function BankExtractTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      <button onClick={handleExtract} disabled={loading || selectedFiles.length === 0} style={{ ...S.primaryBtn(loading || selectedFiles.length === 0), marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      {[
                        ...
                        <div key={col.label} style={{ background: col.match ? '#dcfce7' : col.pdfVal == null ? '#f3f4f6' : '#fee2e2', borderRadius: '8px', padding: '12px' }}>
                          ...
                          <div style={{ fontSize: '20px', fontWeight: '800', marginBottom: '6px', color: col.match ? '#166534' : col.pdfVal == null ? '#9ca3af' : '#991b1b' }}>
                            {col.match ? '✅' : col.pdfVal == null ? '—' : '❌'}
                          </div>
...
// ─────────────────────────────────────────────────────────────
// QC BANK EXTRACTION TOOL — improved layout
// ─────────────────────────────────────────────────────────────
function QCBankExtractionTool({ onBack }) {
  ...
  return (
    <div style={S.card}>
      ...
      {/* Upload Section — properly structured */}
      <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div>
              <label style={S.label}>📊 Excel Files (Extracted Data)</label>
              <UploadZone icon="📊" title="Upload Excel / CSV" subtitle=".xlsx, .xls, .csv" color="#1a3c6e" accept=".xlsx,.xls,.csv" multiple onChange={e => loadExcels(e.target.files)} />
            </div>
            {excelFiles.length > 0 && (
              <FileList
                files={excelFiles}
                selected={selectedExcels}
                onToggle={n => setSelExcels(p => ({ ...p, [n]: !p[n] }))}
                iconFn={() => '📊'}
              />
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div>
              <label style={S.label}>📄 Bank Statement PDFs (Ground Truth)</label>
              <UploadZone icon="📄" title="Upload PDFs" subtitle="Original bank statement PDFs" color="#15803d" accept=".pdf" multiple onChange={e => loadPDFs(e.target.files)} />
            </div>
            {pdfFiles.length > 0 && (
              <FileList
                files={pdfFiles}
                selected={selectedPDFs}
                onToggle={n => setSelPDFs(p => ({ ...p, [n]: !p[n] }))}
                iconFn={() => '📄'}
              />
            )}
          </div>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '14px', paddingTop: '14px', borderTop: '1px solid #e5e7eb' }}>
          <div style={{ flex: 1, background: 'white', borderRadius: '8px', padding: '10px 14px', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <div style={{ fontWeight: '700', fontSize: '14px', color: '#1a3c6e' }}>{selExcelList.length}</div>
              <div style={{ fontSize: '11px', color: '#6b7280' }}>Excel selected</div>
            </div>
          </div>
          <div style={{ flex: 1, background: 'white', borderRadius: '8px', padding: '10px 14px', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '18px' }}>📄</span>
            <div>
              <div style={{ fontWeight: '700', fontSize: '14px', color: '#15803d' }}>{selPDFList.length}</div>
              <div style={{ fontSize: '11px', color: '#6b7280' }}>PDFs selected</div>
            </div>
          </div>
          <div style={{ flex: 2, background: canRun ? '#eff6ff' : '#f8fafc', borderRadius: '8px', padding: '10px 14px', border: `1px solid ${canRun ? '#bfdbfe' : '#e5e7eb'}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <p style={{ margin: 0, fontSize: '11px', color: canRun ? '#1e40af' : '#9ca3af', lineHeight: '1.4' }}>
              {canRun
                ? 'Matching via File Name column. If none found, all rows matched to PDF.'
                : 'Upload at least 1 Excel and 1 PDF to run QC.'}
            </p>
          </div>
        </div>
      </div>
          ...
          {report.results.map((res) => (
            <div key={res.file} style={{ border: '1px solid #e5e7eb', borderRadius: '12px', marginBottom: '16px', overflow: 'hidden' }}>
              <div onClick={() => setExpandedFile(expandedFile === res.file ? null : res.file)}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', background: '#f8fafc', cursor: 'pointer', borderBottom: expandedFile === res.file ? '1px solid #e5e7eb' : 'none', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                  ...
                  <span style={{ background: riskBg(res.riskLevel), color: riskColor(res.riskLevel), border: `1px solid ${riskBorder(res.riskLevel)}`, borderRadius: '20px', padding: '3px 12px', fontSize: '11px', fontWeight: '700' }}>
                  ...
                  {res.insights.length > 0 && (
                    <div style={{ marginBottom: '20px' }}>
                      ...
                      {res.insights.map((ins, i) => (
                        <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '6px', fontSize: '12px', color: '#374151' }}><span style={{ color: '#6b7280' }}>›</span><span>{ins}</span></div>
                      ))}
                    </div>
                  )}
                        ...
                        <thead>
                          <tr style={{ background: '#0f2444' }}>
                            {['Check', 'Status', 'PDF', 'Excel', 'Details'].map(h => (
                              <th key={h} style={{ padding: '10px 12px', textAlign: 'left', color: 'white', fontWeight: '700', fontSize: '11px' }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          <thead>
                            <tr style={{ background: '#0f2444' }}>
                              {['Date', 'Description', 'Issue', 'Expected', 'Extracted'].map(h => (
                                <th key={h} style={{ padding: '9px 12px', textAlign: 'left', color: 'white', fontWeight: '700', fontSize: '11px' }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                  ...
                  {res.categoryChecks.length > 0 && (
                    <div style={{ marginBottom: '20px' }}>
                      ...
                      {res.categoryChecks.map((c, i) => (
                        <div key={i} style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '10px 14px', marginBottom: '8px', fontSize: '12px' }}>
                  ...
                  {res.patterns.length > 0 && (
                    <div style={{ marginBottom: '20px' }}>
                      ...
                      {res.patterns.map((p, i) => (
                        <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '6px', fontSize: '12px', color: '#92400e', background: '#fffbeb', borderRadius: '6px', padding: '8px 12px' }}>
